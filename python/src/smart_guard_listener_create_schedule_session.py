"""
MQTT listener that reacts to RFID verification responses and triggers the
...
LOGGER = logging.getLogger("smart_guard_listener")
...
MQTT_TOPICS = [
    ...
    for topic in os.getenv(
        ...
        ",".join(
            [
                "dJfmRURS5LaJtZ1NZAHX86A9uAk4LZ-smart-guard-rfid-response"
            ]
        ),
    ).split(",")
    if topic.strip()
...
def fetch_user_by_card(card_id: str) -> Optional[Dict[str, Any]]:
    url = f"{API_BASE}/user-rfids/card/{card_id}"
    try:
        resp = requests.get(url, headers=_api_headers(), timeout=10)
        resp.raise_for_status()
    except requests.RequestException as exc:
        LOGGER.error("Failed to fetch RFID data for %s: %s", card_id, exc)
        return None

    payload = resp.json()
    if not payload.get("status"):
        LOGGER.warning("RFID lookup for %s returned status=false: %s", card_id, payload)
        return None
...
def process_rfid_payload(message: Dict[str, Any], client: mqtt.Client) -> None:
    ...
    card_id = str(message.get("data")).strip()
    if not card_id:
    ...
    if role != "FACULTY":
        LOGGER.info("User is not faculty; stopping after RFID verification.")
        return
